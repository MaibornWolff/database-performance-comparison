...
def main(target, config, workers, runs, primary_key, tables, num_inserts, extra_option, timeout, batch):
    worker_counts = map(lambda el: int(el), workers.split(","))
    config = _read_config(config)
    target_config = config["targets"][target]
    namespace = config.get("namespace", "default")
    run_config, target_module = _prepare_run_config(target_config, primary_key, tables, num_inserts, int(batch) if batch else None, extra_option)
...
def _prepare_run_config(target_config, primary_key, tables, num_inserts, batch, extra_options):
